import { callTestnetFunction } from "@/clients/alchemy"
...
export async function getMagicApproved(address: string) {
    ...
    const response = await callTestnetFunction(process.env.NEXT_PUBLIC_MAGIC_ADDRESS!, encoded)
    const decoded = inter.decodeFunctionResult(functionName, response)[0]
    return Number(Web3.utils.fromWei(parseInt(decoded), "ether"))
}
