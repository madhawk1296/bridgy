...
export default function AnswerDetail({ answer, title, color, isPercent=false }: { answer: number, title: string, color: "blue" | "red" | "green" | "yellow", isPercent?: boolean}) {
    const textColor = {
        "red": "text-red-500",
        "blue": "text-blue-500",
    ...
    return (
        <div className={`w-[150px] h-[100px] border-2 ${borderColor[color]} rounded-xl shadow-md flex flex-col items-center justify-center`}>
            <div className={`flex items-center gap-2 text-2xl md:text-3xl tracking-wide text-gray-800 ${rubik.medium} `}>
                {!isPercent && <div className="h-[17px] md:h-[20px]">
                    <Magic />
                </div>}
                {numeral(answer).format(isPercent ? "0,0.0%" : "0,0.0a")}
            </div>
